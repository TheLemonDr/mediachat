...
function draw() {
    for (var nodeIndex = 0; nodeIndex < gameF.nodes.length; nodeIndex++) {
        ...
        if (node.key == 1) {
            drawRect((node.xm - 1) * gridSize, (node.ym - 1) * gridSize, gridSize, gridSize, "#211e20");
        }
    }
...
for (var i = 0; i < gameF.nodes.length; i++) { gameF.nodes[i].key = Math.round(Math.random()) } generations = 0;
